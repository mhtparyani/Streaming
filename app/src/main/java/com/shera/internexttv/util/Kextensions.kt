...
import com.shera.internexttv.startMedialibrary
import java.io.File
import java.net.URI
...
fun asyncTextItem(view: TextView, item: MediaLibraryItem?) {
    ...
    val text = if (item.itemType == MediaLibraryItem.TYPE_PLAYLIST) view.context.getString(com.shera.internexttv.R.string.track_number, item.tracks.size.toString()) else item.description
    if (text.isNullOrEmpty()) {
        view.visibility = View.GONE
        return
    }
